...
def score_candidate(expected, model_chars, eidx, midx, previous, stream, body_min, body_max):
    if midx <= previous:
        return -999.0
    exp_left = "".join(expected[max(0, eidx - 18):eidx])
    exp_right = "".join(expected[eidx + 1:min(len(expected), eidx + 19)])
    ...
    target, desired = model_chars[midx], expected[eidx]
...
def main() -> None:
    ...
    report = {"work_id": "014-02", "page_count": PAGE_COUNT, "status": "started", "colophon_text": COLOPHON}
    try:
        raw_rows = unwrap_rows(json.loads(MODEL.read_text("utf-8")))
        selected = []
        for source_index, row in enumerate(raw_rows):
            if str(row.get("virtual_id", "")) != "014-02" and str(row.get("work_id", "")) != "014-02":
                continue
            ...
            item.update({
                "work_id": "014", "virtual_id": "014-02", "canvas_index": page,
                ...
                "bbox": [box["x"], box["y"], box["w"], box["h"]], "local_image": image_path(page),
            })
        ...
        exact_map, useful_blocks, block_pages = {}, [], []
        for block in blocks:
            if block.size >= 3:
        ...
        for case in cases:
            ...
            if eidx is not None and 0 <= eidx < len(expected):
                ...
                for midx in range(max(previous + 1, center - 160, 0), min(len(model_chars), center + 161)):
                    candidates.append((score_candidate(expected, model_chars, eidx, midx, previous, stream, body_start, body_end), midx, "context-alignment"))
            candidates.sort(key=lambda item: item[0], reverse=True)
            best = candidates[0] if candidates else None
            second = candidates[1] if len(candidates) > 1 else None
            accepted = bool(best and (best[2] == "sequence-exact" or (best[0] >= 1.30 and best[0] - (second[0] if second else -9) >= 0.07)))
            audit = {"id": case.get("id"), "expected_index": eidx, "accepted": accepted, "top_candidates": []}
            for score, midx, method in candidates[:4]:
                row = stream[midx]
                audit["top_candidates"].append({"score": round(score, 4), "method": method, "page": int(row["canvas_index"]), "order_in_page": int(row["order_in_page"]), "char": row.get("char", ""), "glyph_id": row.get("glyph_id", "")})
            if accepted and best:
                score, midx, method = best
                row, box, page = stream[midx], rect(stream[midx]), int(stream[midx]["canvas_index"])
                location = {
                    "page": page, "image": image_path(page), "glyph_id": str(row.get("glyph_id", "")),
                ...
                previous, located = midx, located + 1
        ...
        coordinate_pages, image_only_pages, colophon_pages = [], [], []
        ...
        page_index = json.loads(PAGE_INDEX.read_text("utf-8")); works = page_index.setdefault("works", {})
        pages = []
        for page in range(1, PAGE_COUNT + 1):
            rows = groups.get(page, []); first = rows[0] if rows else {}
            pages.append({"page": page, "label": chinese_page_label(page), "image": image_path(page), "text_clean": "", "text_raw": "", "char_count": len(rows), "has_char_boxes": bool(rows and body_start <= page <= body_end), "canvas_width": int(first.get("canvas_width", 2943) or 2943), "canvas_height": int(first.get("canvas_height", 4429) or 4429)})
        works["014-02"] = {"id": "014-02", "parent_id": "014", "volume_no": 2, "title": "颜真卿李玄靖碑册二", "cover": image_path(1), "page_count": PAGE_COUNT, "pages": pages}
        parent = works.get("014") if isinstance(works.get("014"), dict) else {}
        parent.update({"id": "014", "title": "颜真卿李玄靖碑", "cover": "assets/page_images/014_颜真卿李玄靖碑/images/01_顏真卿李玄靖碑册一/0001_一.jpg", "has_volumes": True, "volumes": [{"id": "014-01", "title": "颜真卿李玄靖碑册一", "page_count": 76}, {"id": "014-02", "title": "颜真卿李玄靖碑册二", "page_count": 72}]})
        works["014"] = parent
        ...
        report.update({"status": "completed", "model_rows": len(selected), "model_stream_chars": len(model_chars), "expected_chars": len(expected), "sequence_ratio": round(matcher.ratio(), 4), "body_page_range": [body_start, body_end], "coordinate_pages": coordinate_pages, "image_only_pages": image_only_pages, "colophon_or_appendix_pages": colophon_pages, "case_count": len(cases), "located_cases": located, "unlocated_cases": [audit["id"] for audit in audits if not audit["accepted"]], "matching_blocks": useful_blocks[:100], "cases": audits})
    except Exception as error:
        report.update({"status": "error", "error": repr(error), "traceback": traceback.format_exc()})
    REPORT.parent.mkdir(parents=True, exist_ok=True)
    REPORT.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps({key: report.get(key) for key in ("status", "model_rows", "sequence_ratio", "body_page_range", "case_count", "located_cases", "unlocated_cases", "colophon_or_appendix_pages", "error")}, ensure_ascii=False, indent=2))
