/* 025《集王羲之书三藏圣教序》栏目一逐页真实坐标适配。 */
(function(){
  "use strict";
  const raw=String(new URLSearchParams(location.search).get("id")||"001");
  const workId=(raw.includes("-")?raw.split("-")[0]:raw).padStart(3,"0");
  if(workId!=="025"||window.__WORK_025_COORDINATE_ADAPTER__)return;
  const CACHE_TAG="20260724_shengjiaoxu_v1";
  const ROOT="data/glyph_boxes/iiif/025";
  ...
  function normalizeRow(row,page,index){
    ...
    return {...row,work_id:"025",canvas_index:pageNo,glyph_id:String(row.glyph_id||`025_${pageNo}_${index+1}`),char:text,text,order_in_page:Number(row.order_in_page||row.annotation_index||index+1),bbox_x:box.x,bbox_y:box.y,bbox_w:box.w,bbox_h:box.h,bbox:[box.x,box.y,box.w,box.h]};
  }
  async function fetchRows(page){
    ...
    const promise=(async()=>{
      ...
      throw lastError||new Error("025坐标读取失败");
    })().catch(error=>{pagePromises.delete(pageNo);console.warn("[work-025-coordinate-adapter]",pageNo,error);return [];});
  ...
  window.loadPageGlyphBoxes=async function(id,pageObj){
    ...
    if(normalized!=="025")return original?original(id,pageObj):[];
  ...
  window.WORK_025_COORDINATES={loadPageRows:fetchRows};
  window.__WORK_025_COORDINATE_ADAPTER__=true;
  window.dispatchEvent(new CustomEvent("work-025-coordinate-adapter-ready"));
})();
