...
(function(){
  ...
  window.__WORK_013_LUJUN__=true;
  ...
  const TITLE="鲁峻碑";
  const VERSION="20260723_lujun_v1";
  const TEXT_URL=`data/work013_full_text.txt?v=${VERSION}`;
  const CASE_URL=`data/work013_damage_cases.json?v=${VERSION}`;
  ...
  function syncCases(items){
    ...
    window.dispatchEvent(new CustomEvent("work-013-cases-ready",{detail:{count:items.length}}));
  }
  ...
  async function renderTranscript(items){
    ...
    try{
      ...
      console.error("[work-013] transcript",error);
  ...
  async function loadPageImages(){
    try{
      ...
      const list=Array.isArray(data?.works?.["013"]?.pages)?data.works["013"].pages:[];
      pageImages=new Map(list.map(page=>[Number(page.page),String(page.image||"")]));
    }catch(error){
      console.warn("[work-013] page images",error);
      pageImages=new Map();
    }
  }
  ...
  const resultLabel=item=>{
    ...
    if(item.mode==="ai_provisional")return "AI暂拟补全";
    return "暂未恢复";
  };
  ...
  function ensureStyle(){
    if(document.getElementById("work013-lujun-style"))return;
    const style=document.createElement("style");
    style.id="work013-lujun-style";
  ...
  async function init(){
    ...
    try{
      ...
      renderDamage();
      window.__WORK_013_CONTENT_READY__=true;
      window.__WORK_013_STABLE_READY__=true;
      window.dispatchEvent(new CustomEvent("work-013-content-ready",{detail:{count:cases.length}}));
      window.dispatchEvent(new CustomEvent("work-013-stable-ready",{detail:{cases:cases.length}}));
    }catch(error){
      console.error("[work-013]",error);
      window.__WORK_013_CONTENT_READY__=true;
      window.__WORK_013_STABLE_READY__=true;
