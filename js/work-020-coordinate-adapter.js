...
(function(){
  ...
  function normalizeRow(row,page,index){
    ...
    return {
      ...row,work_id:"020",canvas_index:pageNo,
      glyph_id:String(row.glyph_id||`020_${pageNo}_${index+1}`),
      char:text,text,
  ...
  window.loadPageGlyphBoxes=async function(id,pageObj){
    ...
    if(normalized!=="020")return original?original(id,pageObj):[];
