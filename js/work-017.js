/* 017《张猛龙碑并阴》栏目二、三、四专属模块。 */
(function(){
  "use strict";

  const raw=String(new URLSearchParams(location.search).get("id")||"001");
  const workId=(raw.includes("-")?raw.split("-")[0]:raw).padStart(3,"0");
  if(workId!=="017"||window.__WORK_017_ZHANGMENGLONG__)return;
  window.__WORK_017_ZHANGMENGLONG__=true;
  ...
  function syncCases(items){
    ...
    window.dispatchEvent(new CustomEvent("work-017-cases-ready",{detail:{count:items.length}}));
  }

  function paragraphHTML(text){
    return String(text||"").replace(/\r\n?/g,"\n").split(/\n\s*\n/)
      .map(part=>part.trim()).filter(Boolean).map(part=>/^(碑阳|碑阴)　/.test(part)?`<h4 class="work017-part-title">${esc(part)}</h4>`:`<p>${esc(part)}</p>`).join("");
  }
  ...
  async function renderTranscript(items){
    ...
    try{
      ...
      console.error("[work-017] transcript",error);
  ...
  function renderDamage(){
    ...
    section.dataset.work017Dedicated="true";
  ...
  function ensureStyle(){
    if(document.getElementById("work017-zhangmenglong-style"))return;
    const style=document.createElement("style");
    style.id="work017-zhangmenglong-style";
  ...
  async function init(){
    ...
    try{
      ...
      renderDamage();
      window.__WORK_017_CONTENT_READY__=true;
      window.__WORK_017_STABLE_READY__=true;
      window.dispatchEvent(new CustomEvent("work-017-content-ready",{detail:{count:cases.length}}));
      window.dispatchEvent(new CustomEvent("work-017-stable-ready",{detail:{cases:cases.length}}));
    }catch(error){
      console.error("[work-017]",error);
      window.__WORK_017_CONTENT_READY__=true;
      window.__WORK_017_STABLE_READY__=true;
