/* 022《王居士砖塔铭》栏目一逐页真实坐标适配。 */
(function(){
  "use strict";

  const raw=String(new URLSearchParams(location.search).get("id")||"001");
  const workId=(raw.includes("-")?raw.split("-")[0]:raw).padStart(3,"0");
  if(workId!=="022"||window.__WORK_022_COORDINATE_ADAPTER__)return;

  const CACHE_TAG="20260724_wangjushi_v1";
  const ROOT="data/glyph_boxes/iiif/022";
  ...
  function normalizeRow(row,page,index){
    ...
    return {
      ...row,work_id:"022",canvas_index:pageNo,
      glyph_id:String(row.glyph_id||`022_${pageNo}_${index+1}`),
      char:text,text,
  ...
  async function fetchRows(page){
    ...
    const promise=(async()=>{
      ...
      throw lastError||new Error("022坐标读取失败");
    })().catch(error=>{
      pagePromises.delete(pageNo);
      console.warn("[work-022-coordinate-adapter]",pageNo,error);
      return [];
    });
  ...
  window.loadPageGlyphBoxes=async function(id,pageObj){
    ...
    if(normalized!=="022")return original?original(id,pageObj):[];
  ...
  window.WORK_022_COORDINATES={loadPageRows:fetchRows};
  window.__WORK_022_COORDINATE_ADAPTER__=true;
  window.dispatchEvent(new CustomEvent("work-022-coordinate-adapter-ready"));
})();
