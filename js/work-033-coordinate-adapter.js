...
(function(){
  ...
  if(workId!=="033"||window.__WORK_033_COORDINATE_ADAPTER__)return;

  const CACHE_TAG="20260725_zhengzuowei_033_v1";
  const ROOT="data/glyph_boxes/iiif/033";
  ...
  function normalizeRow(row,page,index){
    ...
    return {...row,work_id:"033",canvas_index:pageNo,glyph_id:String(row.glyph_id||`033_${pageNo}_${index+1}`),char:text,text,order_in_page:Number(row.order_in_page||row.annotation_index||index+1),bbox_x:box.x,bbox_y:box.y,bbox_w:box.w,bbox_h:box.h,bbox:[box.x,box.y,box.w,box.h]};
  }
  async function fetchRows(page){
    ...
    const promise=(async()=>{
      const response=await fetch(`${ROOT}/page_${String(pageNo).padStart(4,"0")}.json?v=${CACHE_TAG}`,{cache:"force-cache"});
    ...
    })().catch(error=>{pagePromises.delete(pageNo);console.warn("[work-033-coordinate-adapter]",pageNo,error);return[];});
  ...
  window.loadPageGlyphBoxes=async function(id,pageObj){
    ...
    if(normalized!=="033")return originalLoader?originalLoader(id,pageObj):[];
  ...
  window.WORK_033_COORDINATES={loadPageRows:fetchRows};
  window.__WORK_033_COORDINATE_ADAPTER__=true;
  window.dispatchEvent(new CustomEvent("work-033-coordinate-adapter-ready"));
})();
