...
(function(){
  ...
  function normalizeRow(row,page,index){
    ...
    return {
      ...
      virtual_id:"014-01",
      canvas_index:pageNo,
      glyph_id:String(row.glyph_id||`014-01_${pageNo}_${index+1}`),
      char:text,
      text,
  ...
  window.loadPageGlyphBoxes=async function(id,pageObj){
    ...
    if(normalized!=="014-01")return original?original(id,pageObj):[];
  ...
  window.WORK_014_01_COORDINATES={loadPageRows:fetchRows};
  window.__WORK_014_01_COORDINATE_ADAPTER__=true;
  ...
  function refreshReader(attempt=0){
    try{
      ...
      console.warn("[work-014-01-coordinate-adapter] reader refresh",error);
    }
