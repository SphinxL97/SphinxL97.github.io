...
(function(){
  ...
  const INTRO="本栏目对碑文与碑末建碑题记中的十四组残损逐例释读。文献可确认者给出候选字，部分恢复与暂未恢复位置继续保留方框。";
  const IMAGE_ROOT="assets/page_images/023_圭峰定慧禅师碑/images";
  ...
  function syncCases(items){
    ...
    window.dispatchEvent(new CustomEvent("work-023-cases-ready",{detail:{count:items.length}}));
  }

  function paragraphHTML(text){
    return String(text||"").replace(/\r\n?/g,"\n").split(/\n\s*\n/)
      .map(part=>part.trim()).filter(Boolean)
      .map(part=>/^(銘曰|碑末建碑題記|碑额|碑阳|碑阴)/.test(part)?`<h4 class="work023-part-title">${esc(part)}</h4>`:`<p>${esc(part)}</p>`).join("");
  }
  ...
  async function renderTranscript(items){
    ...
    try{
      ...
      console.error("[work-023] transcript",error);
  ...
  function renderDamage(){
    ...
    section.dataset.work023Dedicated="true";
  ...
  async function init(){
    ...
    try{
      ...
      renderDamage();
      window.__WORK_023_CONTENT_READY__=true;
      window.__WORK_023_STABLE_READY__=true;
      window.dispatchEvent(new CustomEvent("work-023-content-ready",{detail:{count:cases.length}}));
      window.dispatchEvent(new CustomEvent("work-023-stable-ready",{detail:{cases:cases.length}}));
    }catch(error){
      console.error("[work-023]",error);
      window.__WORK_023_CONTENT_READY__=true;
      window.__WORK_023_STABLE_READY__=true;
