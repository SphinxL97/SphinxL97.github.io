...
(function(){
  ...
  if(workId!=="034"||window.__WORK_034_COORDINATE_ADAPTER__)return;

  const CACHE_TAG="20260726_zhangjilao_034_v1";
  const ROOT="data/glyph_boxes/iiif/034";
  ...
  function normalizeRow(row,page,index){
    ...
    return {...row,work_id:"034",canvas_index:pageNo,glyph_id:String(row.glyph_id||`034_${pageNo}_${index+1}`),char:text,text,order_in_page:Number(row.order_in_page||row.annotation_index||index+1),bbox_x:box.x,bbox_y:box.y,bbox_w:box.w,bbox_h:box.h,bbox:[box.x,box.y,box.w,box.h]};
  }
  async function fetchRows(page){
    ...
    })().catch(error=>{pagePromises.delete(pageNo);console.warn("[work-034-coordinate-adapter]",pageNo,error);return[];});
  ...
  window.loadPageGlyphBoxes=async function(id,pageObj){
    ...
    if(normalized!=="034")return originalLoader?originalLoader(id,pageObj):[];
  ...
  window.WORK_034_COORDINATES={loadPageRows:fetchRows};
  window.__WORK_034_COORDINATE_ADAPTER__=true;
  window.dispatchEvent(new CustomEvent("work-034-coordinate-adapter-ready"));
})();
