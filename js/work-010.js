/* 010《赵清献公碑》栏目二、三、四专属模块。 */
(function(){
  "use strict";

  const raw=String(new URLSearchParams(location.search).get("id")||"001");
  const workId=(raw.includes("-")?raw.split("-")[0]:raw).padStart(3,"0");
  if(workId!=="010"||window.__WORK_010_ZHAOQINGXIAN__)return;
  window.__WORK_010_ZHAOQINGXIAN__=true;
  ...
  const TITLE="赵清献公碑";
  const TEXT_URL="data/work010_full_text.txt?v=20260722_zhaoqingxian_v1";
  const CASE_URL="data/work010_damage_cases.json?v=20260722_zhaoqingxian_v1";
  const PAGE_INDEX_URL="data/page_images_index.json?v=20260722_zhaoqingxian_v1";
  const NOTE="本节页面展示释文为由AI整理阅读版，段落划分和标点符号由AI辅助校对，仅供阅读参考。";
  const INTRO="本栏目以当前网页释文为底稿，只对原释文中明确标出的缺字提出校读意见。AI分析重点说明候选字与人物官衔、宋代制度、地名人名、固定语汇及上下文的关系；没有对应方框的外部文字不写入恢复结果。";
  ...
  function syncCases(items){
    ...
    window.dispatchEvent(new CustomEvent("work-010-cases-ready",{detail:{count:items.length}}));
  }
  ...
  async function renderTranscript(items){
    ...
    try{
      ...
      console.error("[work-010] transcript",error);
  ...
  async function loadPageImages(){
    try{
      ...
      const list=Array.isArray(data?.works?.["010"]?.pages)?data.works["010"].pages:[];
      pageImages=new Map(list.map(page=>[Number(page.page),String(page.image||"")]));
    }catch(error){
      console.warn("[work-010] page images",error);
      pageImages=new Map();
    }
  }
  ...
  function renderDamage(){
    ...
    section.dataset.work010Dedicated="true";
  ...
  function ensureStyle(){
    if(document.getElementById("work010-zhaoqingxian-style"))return;
    const style=document.createElement("style");
    style.id="work010-zhaoqingxian-style";
  ...
  async function init(){
    ...
    try{
      ...
      renderDamage();
      window.__WORK_010_CONTENT_READY__=true;
      window.__WORK_010_STABLE_READY__=true;
      window.dispatchEvent(new CustomEvent("work-010-content-ready",{detail:{count:cases.length}}));
      window.dispatchEvent(new CustomEvent("work-010-stable-ready",{detail:{cases:cases.length}}));
    }catch(error){
      console.error("[work-010]",error);
      window.__WORK_010_CONTENT_READY__=true;
      window.__WORK_010_STABLE_READY__=true;
