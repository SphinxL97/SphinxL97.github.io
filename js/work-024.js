...
(function () {
  ...
  const TITLE = "张从申书李玄靖碑";
  const VERSION = "20260724_xuanjing_v2";
  const TEXT_URL = `data/work024_full_text.txt?v=${VERSION}`;
  const CASE_URL = `data/work024_damage_cases.json?v=${VERSION}`;
  const IMAGE_ROOT = "assets/page_images/024_张从申书李玄靖碑/images";
  ...
  function publishCases(items) {
    ...
    window.dispatchEvent(new CustomEvent("work-024-cases-ready", {
  ...
  function paragraphHTML(text) {
    ...
    return normalized
      ...
      .map((part) => /^文曰[:：]?/.test(part)
        ? `<h4 class="work024-part-title">${escapeHTML(part)}</h4>`
        : `<p>${escapeHTML(part)}</p>`)
      .join("");
  }
  ...
  function renderDamage() {
    ...
    section.dataset.work024Dedicated = "true";
    section.innerHTML = `
  ...
  function ensureCrowdsource() {
    ...
    if (window.__CROWDSOURCE_MISSING_V10__) {
      window.__WORK_024_CROWDSOURCE_READY__ = true;
      return;
    }
    ...
    if (!Array.from(document.scripts).some((script) => (script.getAttribute("src") || "").split("?")[0].endsWith(scriptPath))) {
      ...
      script.addEventListener("load", () => {
        window.__WORK_024_CROWDSOURCE_READY__ = true;
        window.dispatchEvent(new CustomEvent("work-024-crowdsource-ready", { detail: { count: cases.length } }));
      }, { once: true });
  ...
  async function init() {
    ...
    try {
      ...
      if (!cases.length) throw new Error("024案例数据为空");
      publishCases(cases);
      renderTranscript(text, cases);
      renderDamage();
      ensureCrowdsource();
      window.__WORK_024_CONTENT_READY__ = true;
      window.__WORK_024_STABLE_READY__ = true;
      window.dispatchEvent(new CustomEvent("work-024-content-ready", { detail: { count: cases.length } }));
      window.dispatchEvent(new CustomEvent("work-024-stable-ready", { detail: { cases: cases.length } }));
    } catch (error) {
      console.error("[work-024]", error);
      const transcript = document.getElementById("calligraphy");
      const damage = document.getElementById("people");
      if (transcript) transcript.innerHTML = '<h2 class="section-title">二、碑文释文</h2><div class="full-transcript-error">024碑文数据读取失败，请刷新页面后重试。</div>';
      if (damage) damage.innerHTML = '<h2 class="section-title">三、碑文残损与AI释读</h2><div class="full-transcript-error">024案例数据读取失败，请刷新页面后重试。</div>';
    }
  }
