/* 011《皇甫诞碑》栏目二、三、四专属模块。 */
(function(){
  "use strict";

  const raw=String(new URLSearchParams(location.search).get("id")||"001");
  const workId=(raw.includes("-")?raw.split("-")[0]:raw).padStart(3,"0");
  if(workId!=="011"||window.__WORK_011_HUANGFUDAN__)return;
  window.__WORK_011_HUANGFUDAN__=true;
  ...
  const TITLE="皇甫诞碑";
  const VERSION="20260723_huangfudan_v1";
  const TEXT_URL=`data/work011_full_text.txt?v=${VERSION}`;
  const CASE_URL=`data/work011_damage_cases.json?v=${VERSION}`;
  const PAGE_INDEX_URL=`data/page_images_index.json?v=${VERSION}`;
  const NOTE="本节页面展示释文为由AI整理阅读版，段落划分和标点符号由AI辅助校对，仅供阅读参考。";
  const INTRO="本栏目以当前网页释文为底稿，只对原释文中明确标出的五处残损提出校读意见。AI分析重点说明候选字与典故、古代制度、对偶结构及上下文的关系；没有对应方框的外部文字不写入恢复结果。";
  ...
  function syncCases(items){
    ...
    window.dispatchEvent(new CustomEvent("work-011-cases-ready",{detail:{count:items.length}}));
  }
  ...
  async function renderTranscript(items){
    ...
    try{
      ...
      console.error("[work-011] transcript",error);
  ...
  async function loadPageImages(){
    try{
      ...
      const list=Array.isArray(data?.works?.["011"]?.pages)?data.works["011"].pages:[];
      pageImages=new Map(list.map(page=>[Number(page.page),String(page.image||"")]));
    }catch(error){
      console.warn("[work-011] page images",error);
      pageImages=new Map();
    }
  }
  ...
  function renderDamage(){
    ...
    section.dataset.work011Dedicated="true";
  ...
  function ensureStyle(){
    if(document.getElementById("work011-huangfudan-style"))return;
    const style=document.createElement("style");
    style.id="work011-huangfudan-style";
  ...
  async function init(){
    ...
    try{
      ...
      renderDamage();
      window.__WORK_011_CONTENT_READY__=true;
      window.__WORK_011_STABLE_READY__=true;
      window.dispatchEvent(new CustomEvent("work-011-content-ready",{detail:{count:cases.length}}));
      window.dispatchEvent(new CustomEvent("work-011-stable-ready",{detail:{cases:cases.length}}));
    }catch(error){
      console.error("[work-011]",error);
      window.__WORK_011_CONTENT_READY__=true;
      window.__WORK_011_STABLE_READY__=true;
