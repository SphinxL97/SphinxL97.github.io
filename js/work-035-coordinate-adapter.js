...
(function(){
  ...
  const CACHE_TAG="20260726_wushici_035_v1";
  ...
  function normalizeRow(row,page,index){
    ...
    return {...row,work_id:"035",canvas_index:pageNo,glyph_id:String(row.glyph_id||`035_${pageNo}_${index+1}`),char:text,text,order_in_page:Number(row.order_in_page||row.annotation_index||index+1),bbox_x:box.x,bbox_y:box.y,bbox_w:box.w,bbox_h:box.h,bbox:[box.x,box.y,box.w,box.h]};
  }
  async function fetchRows(page){
    ...
    })().catch(error=>{pagePromises.delete(pageNo);console.warn("[work-035-coordinate-adapter]",pageNo,error);return[];});
  ...
  window.loadPageGlyphBoxes=async function(id,pageObj){
    ...
    if(normalized!=="035")return originalLoader?originalLoader(id,pageObj):[];
  ...
  window.WORK_035_COORDINATES={loadPageRows:fetchRows};
  window.__WORK_035_COORDINATE_ADAPTER__=true;
  window.dispatchEvent(new CustomEvent("work-035-coordinate-adapter-ready"));
})();
