/* 022《王居士砖塔铭》栏目二、三、四专属模块。 */
(function(){
  "use strict";

  const raw=String(new URLSearchParams(location.search).get("id")||"001");
  const workId=(raw.includes("-")?raw.split("-")[0]:raw).padStart(3,"0");
  if(workId!=="022"||window.__WORK_022_WANGJUSHI__)return;
  window.__WORK_022_WANGJUSHI__=true;
  ...
  const TITLE="王居士砖塔铭";
  const VERSION="20260724_wangjushi_v1";
  const TEXT_URL=`data/work022_full_text.txt?v=${VERSION}`;
  const CASE_URL=`data/work022_damage_cases.json?v=${VERSION}`;
  ...
  const IMAGE_ROOT="assets/page_images/022_王居士砖塔铭/images";
  ...
  function syncCases(items){
    ...
    window.dispatchEvent(new CustomEvent("work-022-cases-ready",{detail:{count:items.length}}));
  }

  function paragraphHTML(text){
    return String(text||"").replace(/\r\n?/g,"\n").split(/\n\s*\n/)
      .map(part=>part.trim()).filter(Boolean)
      .map(part=>/^(（一）|（二）|碑额|碑阳|碑阴)/.test(part)?`<h4 class="work022-part-title">${esc(part)}</h4>`:`<p>${esc(part)}</p>`).join("");
  }
  ...
  async function renderTranscript(items){
    ...
    try{
      ...
      console.error("[work-022] transcript",error);
  ...
  function renderDamage(){
    ...
    section.dataset.work022Dedicated="true";
  ...
  async function init(){
    ...
    try{
      ...
      renderDamage();
      window.__WORK_022_CONTENT_READY__=true;
      window.__WORK_022_STABLE_READY__=true;
      window.dispatchEvent(new CustomEvent("work-022-content-ready",{detail:{count:cases.length}}));
      window.dispatchEvent(new CustomEvent("work-022-stable-ready",{detail:{cases:cases.length}}));
    }catch(error){
      console.error("[work-022]",error);
      window.__WORK_022_CONTENT_READY__=true;
      window.__WORK_022_STABLE_READY__=true;
