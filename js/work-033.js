/* 033《争座位帖》栏目二、三、四专属模块。 */
(function(){
  "use strict";
  const raw=String(new URLSearchParams(location.search).get("id")||"001");
  const workId=(raw.includes("-")?raw.split("-")[0]:raw).padStart(3,"0");
  if(workId!=="033"||window.__WORK_033_ZHENGZUOWEI__)return;
  window.__WORK_033_ZHENGZUOWEI__=true;
  ...
  const TITLE="争座位帖";
  const VERSION="20260725_zhengzuowei_033_v1";
  const TEXT_URL=`data/work033_full_text.txt?v=${VERSION}`;
  const CASE_URL=`data/work033_damage_cases.json?v=${VERSION}`;
  const PAGE_INDEX_URL=`data/page_images_index.json?v=${VERSION}`;
  const NOTE="本节页面展示释文为由AI整理阅读版，段落划分和标点符号由AI辅助校对，仅供阅读参考。";
  const INTRO="本栏目依据用户确认的底稿、可靠录文与仓库既有模型字框，展示三处残损补字和一处形近字纠错。栏目二保留底稿原貌；栏目三以〔〕标示校订结果。4例中3例已定位真实字框，1例仅能确定所在页，不估算bbox。";
  ...
  function publishCases(items){
    ...
    window.dispatchEvent(new CustomEvent("work-033-cases-ready",{detail:{count:items.length}}));
  }
  ...
  function locationHTML(item){
    ...
    if(!loc?.bbox)return `<div class="work033-page-only"><img src="${esc(image)}" alt="第${page}页原拓"><p>第${page}页可核验，但模型未检测到缺失“之”字的独立字框；本例不估算bbox。</p></div>`;
    ...
    return `<div class="work033-case-image"><div class="work033-image-stage"><img src="${esc(image)}" alt="第${page}页原拓"><span class="work033-real-box" style="left:${left}%;top:${top}%;width:${width}%;height:${height}%" title="${esc(loc.glyph_id||"")}"></span></div><p>第${page}页 · 真实模型字框 ${esc(loc.glyph_id||"")}</p></div>`;
  }
  function renderDamage(){
    ...
    setMenuTitle(3,"三、碑文残损与AI释读");publishCases(cases);section.className="content-card damage-ai";section.dataset.work033Dedicated="true";
  ...
  function ensureStyle(){
    if(document.getElementById("work033-zhengzuowei-style"))return;const style=document.createElement("style");style.id="work033-zhengzuowei-style";
    style.textContent=".damage-heading-confidence{font-size:.78em;color:#675b4e;white-space:nowrap}.damage-added{padding:0 .12em;border-bottom:2px solid #a53529;border-radius:4px;background:#f8e1cf;color:#9f3025!important;font-weight:900}.damage-text.damage-new{color:#2e251e!important;font-weight:400!important}.work033-image-stage{position:relative;width:min(100%,430px);margin:auto}.work033-image-stage img,.work033-page-only img{width:100%;height:auto;display:block;border-radius:10px}.work033-real-box{position:absolute;border:3px solid #e23020;background:rgba(226,48,32,.12);box-shadow:0 0 0 2px rgba(255,255,255,.8)}.work033-case-image p,.work033-page-only p{margin:10px 0 0!important;text-indent:0!important;font-size:12px;color:#766657;text-align:center}.work033-page-only{padding:12px;border:1px dashed #d8c69f;border-radius:14px;background:#fffaf0}.damage-location-missing{display:flex;align-items:center;justify-content:center;min-height:250px;padding:30px;border:1px dashed #d8c69f;border-radius:14px;background:#fffaf0;color:#7b6c5a;text-align:center}";
    document.head.appendChild(style);
  }
  function applySupplementalInfo(){
    const alias=document.querySelector(".info-panel .alias");if(alias)alias.textContent="唐广德二年（764）十一月，颜真卿写给右仆射郭英乂、申明朝廷班秩与礼制秩序的书信稿。";
    const triggers=document.querySelectorAll(".info-trigger");
    if(triggers[0]){triggers[0].querySelector("strong")?.replaceChildren(document.createTextNode("版本与流传"));triggers[0].querySelector("span:last-child")?.replaceChildren(document.createTextNode("上海图书馆藏宋拓本，装裱8开，数字图像42页。"));}
    if(triggers[1]){triggers[1].querySelector("strong")?.replaceChildren(document.createTextNode("争座故事"));triggers[1].querySelector("span:last-child")?.replaceChildren(document.createTextNode("颜真卿借宴会座次之争，直陈朝廷尊卑班秩不可紊乱。"));}
    const history=document.querySelector("#modal-history .modal-body");if(history)history.innerHTML='<div class="modal-two-col"><div class="modal-term">版本说明</div><div class="modal-desc">本项目采用上海图书馆藏宋拓本，装裱8开；网站42页为数字化图像数，二者不是同一计数口径。</div><div class="modal-term">馆藏号</div><div class="modal-desc">19A379。</div></div>';
    const story=document.querySelector("#modal-story .modal-body");if(story)story.innerHTML='<h3>书信背景</h3><p>广德二年十一月，颜真卿致书郭英乂，批评宴会座次混乱、武将与宦官凌越朝廷班秩的做法。全文不是单纯争夺个人座位，而是借礼制秩序申明朝廷纪纲。</p><h3>作品特点</h3><p>书稿在激切议论中保存自然行书笔意，后世因其内容与书法并重，通称《争座位帖》。</p>';
  }
  function ensureCrowdsource(){
    ...
    if(window.__CROWDSOURCE_MISSING_V10__){window.__WORK_033_CROWDSOURCE_READY__=true;return;}
    const scriptPath="assets/js/crowdsource-v9.js";if(!Array.from(document.scripts).some(script=>(script.getAttribute("src")||"").split("?")[0].endsWith(scriptPath))){const script=document.createElement("script");script.src=`${scriptPath}?v=${VERSION}`;script.async=false;script.addEventListener("load",()=>{window.__WORK_033_CROWDSOURCE_READY__=true;window.dispatchEvent(new CustomEvent("work-033-crowdsource-ready",{detail:{count:cases.length}}));},{once:true});document.head.appendChild(script);}else window.__WORK_033_CROWDSOURCE_READY__=true;
  }
  async function init(){
