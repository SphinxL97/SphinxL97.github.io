...
(function(){
  ...
  const CACHE_TAG="20260723_lujun_v1";
  const ROOT="data/glyph_boxes/iiif/013";
  ...
  function normalizeRow(row,page,index){
    ...
    return {
      ...
      work_id:"013",
      canvas_index:pageNo,
      glyph_id:String(row.glyph_id||`013_${pageNo}_${index+1}`),
      char:text,
      text,
  ...
  async function fetchRows(page){
    ...
    const promise=(async()=>{
      ...
      console.warn("[work-013-coordinate-adapter]",pageNo,error);
      return [];
    });
  ...
  window.WORK_013_COORDINATES={loadPageRows:fetchRows};
  window.__WORK_013_COORDINATE_ADAPTER__=true;
  ...
  function refreshReader(attempt=0){
    try{
      ...
      console.warn("[work-013-coordinate-adapter] reader refresh",error);
    }
