/* 020《化度寺邕禅师舍利塔铭》栏目二、三、四专属模块。 */
(function(){
  "use strict";

  const raw=String(new URLSearchParams(location.search).get("id")||"001");
  const workId=(raw.includes("-")?raw.split("-")[0]:raw).padStart(3,"0");
  if(workId!=="020"||window.__WORK_020_HUADUSI__)return;
  window.__WORK_020_HUADUSI__=true;
  ...
  function syncCases(items){
    ...
    window.dispatchEvent(new CustomEvent("work-020-cases-ready",{detail:{count:items.length}}));
  }
  ...
  async function renderTranscript(items){
    ...
    try{
      ...
      console.error("[work-020] transcript",error);
  ...
  function renderDamage(){
    ...
    section.dataset.work020Dedicated="true";
  ...
  function ensureStyle(){
    if(document.getElementById("work020-huadusi-style"))return;
    const style=document.createElement("style");
    style.id="work020-huadusi-style";
  ...
  async function init(){
    ...
    try{
      ...
      renderDamage();
      window.__WORK_020_CONTENT_READY__=true;
      window.__WORK_020_STABLE_READY__=true;
      window.dispatchEvent(new CustomEvent("work-020-content-ready",{detail:{count:cases.length}}));
      window.dispatchEvent(new CustomEvent("work-020-stable-ready",{detail:{cases:cases.length}}));
    }catch(error){
      console.error("[work-020]",error);
      window.__WORK_020_CONTENT_READY__=true;
      window.__WORK_020_STABLE_READY__=true;
