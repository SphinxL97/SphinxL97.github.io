/* 018《中岳嵩高灵庙碑并额》栏目一逐页真实坐标适配。 */
(function(){
  "use strict";

  const raw=String(new URLSearchParams(location.search).get("id")||"001");
  const workId=(raw.includes("-")?raw.split("-")[0]:raw).padStart(3,"0");
  if(workId!=="018"||window.__WORK_018_COORDINATE_ADAPTER__)return;

  const CACHE_TAG="20260724_songgao_v1";
  ...
  function normalizeRow(row,page,index){
    ...
    return {
      ...row,work_id:"018",canvas_index:pageNo,
      glyph_id:String(row.glyph_id||`018_${pageNo}_${index+1}`),
      char:text,text,
  ...
  async function fetchRows(page){
    ...
    const promise=(async()=>{
      ...
      console.warn("[work-018-coordinate-adapter]",pageNo,error);
      return [];
    });
  ...
  window.loadPageGlyphBoxes=async function(id,pageObj){
    ...
    if(normalized!=="018")return original?original(id,pageObj):[];
  ...
  window.WORK_018_COORDINATES={loadPageRows:fetchRows};
  window.__WORK_018_COORDINATE_ADAPTER__=true;
  ...
  function refreshReader(attempt=0){
    try{
      ...
      console.warn("[work-018-coordinate-adapter] reader refresh",error);
    }
