/* 015《史晨前碑》栏目一逐页真实坐标适配。 */
(function(){
  "use strict";

  const raw=String(new URLSearchParams(location.search).get("id")||"001");
  const workId=(raw.includes("-")?raw.split("-")[0]:raw).padStart(3,"0");
  if(workId!=="015"||window.__WORK_015_COORDINATE_ADAPTER__)return;

  const CACHE_TAG="20260723_shichenqian_v1";
  const ROOT="data/glyph_boxes/iiif/015";
  ...
  function normalizeRow(row,page,index){
    ...
    return {
      ...row,work_id:"015",canvas_index:pageNo,
      glyph_id:String(row.glyph_id||`015_${pageNo}_${index+1}`),
      char:text,text,
  ...
  async function fetchRows(page){
    ...
    const promise=(async()=>{
      ...
      throw lastError||new Error("015坐标读取失败");
    })().catch(error=>{
      pagePromises.delete(pageNo);
      console.warn("[work-015-coordinate-adapter]",pageNo,error);
      return [];
    });
  ...
  window.loadPageGlyphBoxes=async function(id,pageObj){
    ...
    if(normalized!=="015")return original?original(id,pageObj):[];
  ...
  window.WORK_015_COORDINATES={loadPageRows:fetchRows};
  window.__WORK_015_COORDINATE_ADAPTER__=true;
  ...
  function refreshReader(attempt=0){
    try{
      ...
      console.warn("[work-015-coordinate-adapter] reader refresh",error);
    }
