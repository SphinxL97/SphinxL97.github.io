/* 016《上尊号碑受禅表合册》栏目二、三、四专属模块。 */
(function(){
  "use strict";

  const raw=String(new URLSearchParams(location.search).get("id")||"001");
  const workId=(raw.includes("-")?raw.split("-")[0]:raw).padStart(3,"0");
  if(workId!=="016"||window.__WORK_016_SHICHENQIAN__)return;
  window.__WORK_016_SHICHENQIAN__=true;
  ...
  const TITLE="上尊号碑受禅表合册";
  const VERSION="20260723_zunhao_shanrang_v1";
  const TEXT_URL=`data/work016_full_text.txt?v=${VERSION}`;
  const CASE_URL=`data/work016_damage_cases.json?v=${VERSION}`;
  const NOTE="本节页面展示释文为由AI整理阅读版，段落划分和标点符号由AI辅助校对，仅供阅读参考。";
  const INTRO="本栏目以当前网页释文为底稿，分别整理《魏上尊号碑》与《魏受禅表》节录中的残损字。AI分析重点说明候选字与官职封爵、禅让典故、受禅礼制、祥瑞语汇和句法对偶的关系；没有对应方框的外部文字不写入恢复结果。";
  const IMAGE_ROOT="assets/page_images/016_上尊号碑受禅表合册/images";
  ...
  function syncCases(items){
    ...
    window.dispatchEvent(new CustomEvent("work-016-cases-ready",{detail:{count:items.length}}));
  }

  function paragraphHTML(text){
    return String(text||"").replace(/\r\n?/g,"\n").split(/\n\s*\n/)
      .map(part=>part.trim()).filter(Boolean).map(part=>/^[上下]篇　/.test(part)?`<h4 class="work016-part-title">${esc(part)}</h4>`:`<p>${esc(part)}</p>`).join("");
  }
  ...
  async function renderTranscript(items){
    ...
    try{
      ...
      console.error("[work-016] transcript",error);
  ...
  function renderDamage(){
    ...
    section.dataset.work016Dedicated="true";
  ...
  function ensureStyle(){
    if(document.getElementById("work016-zunhaoshanrang-style"))return;
    const style=document.createElement("style");
    style.id="work016-zunhaoshanrang-style";
  ...
  async function init(){
    ...
    try{
      ...
      renderDamage();
      window.__WORK_016_CONTENT_READY__=true;
      window.__WORK_016_STABLE_READY__=true;
      window.dispatchEvent(new CustomEvent("work-016-content-ready",{detail:{count:cases.length}}));
      window.dispatchEvent(new CustomEvent("work-016-stable-ready",{detail:{cases:cases.length}}));
    }catch(error){
      console.error("[work-016]",error);
      window.__WORK_016_CONTENT_READY__=true;
      window.__WORK_016_STABLE_READY__=true;
