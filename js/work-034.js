/* 034《章吉老墓志》栏目二、三、四专属模块。 */
(function(){
  "use strict";
  const raw=String(new URLSearchParams(location.search).get("id")||"001");
  const workId=(raw.includes("-")?raw.split("-")[0]:raw).padStart(3,"0");
  if(workId!=="034"||window.__WORK_034_ZHANGJILAO__)return;
  window.__WORK_034_ZHANGJILAO__=true;
  ...
  function publishCases(items){
    ...
    window.dispatchEvent(new CustomEvent("work-034-cases-ready",{detail:{count:items.length}}));
  }
  ...
  function ensureCrowdsource(){
    ...
    if(window.__CROWDSOURCE_MISSING_V10__){window.__WORK_034_CROWDSOURCE_READY__=true;return;}
  ...
  async function init(){
    ...
    try{
      ...
      await renderTranscript(cases);renderDamage();ensureCrowdsource();window.__WORK_034_CONTENT_READY__=true;window.__WORK_034_STABLE_READY__=true;window.dispatchEvent(new CustomEvent("work-034-stable-ready",{detail:{cases:cases.length}}));
    }catch(error){console.error("[work-034]",error);const transcript=document.getElementById("calligraphy");if(transcript)transcript.innerHTML='<h2 class="section-title">二、碑文释文</h2><div class="full-transcript-error">034碑文数据读取失败，请刷新页面后重试。</div>';if(damage)damage.innerHTML='<h2 class="section-title">三、碑文残损与AI释读</h2><div class="full-transcript-error">034案例数据读取失败，请刷新页面后重试。</div>';window.__WORK_034_CROWDSOURCE_READY__=true;window.__WORK_034_STABLE_READY__=true;}
  }
