/* 010《赵清献公碑》栏目一逐页真实坐标适配。 */
(function(){
  "use strict";

  const raw=String(new URLSearchParams(location.search).get("id")||"001");
  const workId=(raw.includes("-")?raw.split("-")[0]:raw).padStart(3,"0");
  if(workId!=="010"||window.__WORK_010_COORDINATE_ADAPTER__)return;

  const CACHE_TAG="20260722_zhaoqingxian_v1";
  const ROOT="data/glyph_boxes/iiif/010";
  ...
  function normalizeRow(row,page,index){
    ...
    return {
      ...
      work_id:"010",
      canvas_index:pageNo,
      glyph_id:String(row.glyph_id||`010_${pageNo}_${index+1}`),
      char:text,
      text,
  ...
  async function fetchRows(page){
    ...
    const promise=(async()=>{
      ...
      throw lastError||new Error("010坐标读取失败");
    })().catch(error=>{
      pagePromises.delete(pageNo);
      console.warn("[work-010-coordinate-adapter]",pageNo,error);
      return [];
    });
  ...
  window.loadPageGlyphBoxes=async function(id,pageObj){
    ...
    if(normalized!=="010")return original?original(id,pageObj):[];
  ...
  window.WORK_010_COORDINATES={loadPageRows:fetchRows};
  window.__WORK_010_COORDINATE_ADAPTER__=true;
  ...
  function refreshReader(attempt=0){
    try{
      ...
      console.warn("[work-010-coordinate-adapter] reader refresh",error);
    }
