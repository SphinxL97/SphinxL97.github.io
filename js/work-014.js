/* 014-01《颜真卿李玄靖碑册一》栏目二、三、四专属模块。 */
(function(){
  "use strict";

  const raw=String(new URLSearchParams(location.search).get("id")||"001");
  if(raw!=="014-01"||window.__WORK_014_01_LIHANJING__)return;
  window.__WORK_014_01_LIHANJING__=true;
  ...
  function syncCases(items){
    ...
    window.dispatchEvent(new CustomEvent("work-014-01-cases-ready",{detail:{count:items.length}}));
  }
  ...
  async function renderTranscript(items){
    ...
    try{
      ...
      console.error("[work-014-01] transcript",error);
  ...
  function ensureStyle(){
    if(document.getElementById("work014-01-lihanjing-style"))return;
    const style=document.createElement("style");
    style.id="work014-01-lihanjing-style";
  ...
  async function init(){
    ...
    try{
      ...
      window.dispatchEvent(new CustomEvent("work-014-01-stable-ready",{detail:{cases:cases.length}}));
    }catch(error){
      console.error("[work-014-01]",error);
      window.__WORK_014_01_CONTENT_READY__=true;
      window.__WORK_014_01_STABLE_READY__=true;
