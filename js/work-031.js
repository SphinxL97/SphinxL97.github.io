/* 031-01《黄庭堅青原山诗刻石(第一辑)册一》栏目二、三、四专属模块。 */
(function(){
  "use strict";
  const raw=String(new URLSearchParams(location.search).get("id")||"001");
  if(raw!=="031-01"||window.__WORK_031_01_HUANGTINGJIAN__)return;
  window.__WORK_031_01_HUANGTINGJIAN__=true;
  ...
  const TITLE="黄庭堅青原山诗刻石(第一辑)册一";
  const VERSION="20260727_huangtingjian_031_01_v1";
  const TEXT_URL=`data/work031_01_full_text.txt?v=${VERSION}`;
  const CASE_URL=`data/work031_01_damage_cases.json?v=${VERSION}`;
  const PAGE_ROOT="assets/page_images/031_黄庭堅青原山诗刻石/images/01_黄庭堅青原山诗刻石(第一辑)册一";
  const PAGE_COUNT=61;
  const NOTE="本节页面展示释文为由AI整理阅读版，段落划分和标点符号由AI辅助校对，仅供阅读参考。";
  const INTRO="本栏目整理《黄庭堅青原山诗刻石(第一辑)册一》5组残损文字校读。";
  ...
  function publishCases(items){
    ...
    window.dispatchEvent(new CustomEvent("work-031-01-cases-ready",{detail:{count:items.length}}));
  }
  ...
  function ensureStyle(){
    if(document.getElementById("work-031-01-style"))return;const style=document.createElement("style");style.id="work-031-01-style";
  ...
  function ensureCrowdsource(){
    ...
    if(window.__CROWDSOURCE_MISSING_V10__){window.__WORK_031_01_CROWDSOURCE_READY__=true;return;}
    const scriptPath="assets/js/crowdsource-v9.js";if(!Array.from(document.scripts).some(script=>(script.getAttribute("src")||"").split("?")[0].endsWith(scriptPath))){const script=document.createElement("script");script.src=`${scriptPath}?v=${VERSION}`;script.async=false;script.addEventListener("load",()=>{window.__WORK_031_01_CROWDSOURCE_READY__=true;window.dispatchEvent(new CustomEvent("work-031-01-crowdsource-ready",{detail:{count:cases.length}}));},{once:true});document.head.appendChild(script);}else window.__WORK_031_01_CROWDSOURCE_READY__=true;
  }
  async function init(){
    ...
    try{
      const rows=await fetchJSON(CASE_URL);cases=(Array.isArray(rows)?rows:[]).map(normalizeCase);if(cases.length!==5)throw new Error("031-01案例数据数量异常");
      publishCases(cases);await renderTranscript(cases);renderDamage();ensureCrowdsource();window.__WORK_031_01_CONTENT_READY__=true;window.__WORK_031_01_STABLE_READY__=true;window.dispatchEvent(new CustomEvent("work-031-01-stable-ready",{detail:{cases:cases.length}}));
    }catch(error){console.error("[work-031-01]",error);const transcript=document.getElementById("calligraphy");if(transcript)transcript.innerHTML='<h2 class="section-title">二、碑文释文</h2><div class="full-transcript-error">碑文数据读取失败，请刷新页面后重试。</div>';if(damage)damage.innerHTML='<h2 class="section-title">三、碑文残损与AI释读</h2><div class="full-transcript-error">案例数据读取失败，请刷新页面后重试。</div>';window.__WORK_031_01_CROWDSOURCE_READY__=true;window.__WORK_031_01_STABLE_READY__=true;}
  }
