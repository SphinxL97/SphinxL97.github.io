...
(function(){
  ...
  const CACHE_TAG="20260725_xianyu_v2";
  ...
  function normalizeRow(row,page,index){const box=rect(row);if(box.w<=0||box.h<=0)return null;const pageNo=Number(row.canvas_index||row.page||page||0);if(!pageNo)return null;const text=String(row.char||row.text||"").slice(0,1);return{...row,work_id:"029",canvas_index:pageNo,glyph_id:String(row.glyph_id||`029_${pageNo}_${index+1}`),char:text,text,order_in_page:Number(row.order_in_page||row.annotation_index||index+1),bbox_x:box.x,bbox_y:box.y,bbox_w:box.w,bbox_h:box.h,bbox:[box.x,box.y,box.w,box.h]};}
  async function fetchRows(page){const pageNo=Number(page||0);if(!pageNo)return[];if(pagePromises.has(pageNo))return pagePromises.get(pageNo);const promise=(async()=>{const response=await fetch(`${ROOT}/page_${String(pageNo).padStart(4,"0")}.json?v=${CACHE_TAG}`,{cache:"force-cache"});if(response.status===404)return[];if(!response.ok)throw new Error(`${response.status} ${response.statusText}`);const rows=await response.json();return(Array.isArray(rows)?rows:[]).map((row,index)=>normalizeRow(row,pageNo,index)).filter(Boolean).sort((a,b)=>a.order_in_page-b.order_in_page);})().catch(error=>{pagePromises.delete(pageNo);console.warn("[work-029-coordinate-adapter]",pageNo,error);return[];});pagePromises.set(pageNo,promise);return promise;}
  window.loadPageGlyphBoxes=async function(id,pageObj){const normalized=String(id||"").match(/^(\d{3})/)?.[1]||String(id||"").padStart(3,"0");if(normalized!=="029")return originalLoader?originalLoader(id,pageObj):[];const page=Number(pageObj?.canvas_index||pageObj?.page||0);const rows=(await fetchRows(page)).map(row=>({...row,local_image:pageObj?.image||row.local_image||""}));return rows.length?rows:(originalLoader?originalLoader(id,pageObj):[]);};
