...
(function(){
  ...
  function normalizeRow(row,page,index){
    ...
    return {
      ...row,work_id:"023",canvas_index:pageNo,
      glyph_id:String(row.glyph_id||`023_${pageNo}_${index+1}`),
      char:text,text,
  ...
  async function fetchRows(page){
    ...
    const promise=(async()=>{
      ...
      console.warn("[work-023-coordinate-adapter]",pageNo,error);
      return [];
    });
  ...
  window.loadPageGlyphBoxes=async function(id,pageObj){
    ...
    if(normalized!=="023")return original?original(id,pageObj):[];
  ...
  window.WORK_023_COORDINATES={loadPageRows:fetchRows};
  window.__WORK_023_COORDINATE_ADAPTER__=true;
  window.dispatchEvent(new CustomEvent("work-023-coordinate-adapter-ready"));
})();
