/* 007《伊阙佛龛碑》栏目一逐字坐标适配。
 * detail.html 默认读取逐页 IIIF 文件；007现有坐标保存在006—010分片中。
 * 本脚本只替换007的逐页坐标读取函数，不影响其他碑帖。
 */
(function(){
  "use strict";
  ...
  if(parentId!=="007"||window.__WORK_007_COORDINATE_ADAPTER__)return;
  window.__WORK_007_COORDINATE_ADAPTER__=true;

  const MODEL_URL="data/model_boxes/glyph_model_border_006_010.json?v=20260720_work007_v1";
  ...
  function loadRows(){
    if(!rowsPromise){
      rowsPromise=fetch(MODEL_URL,{cache:"force-cache"}).then(response=>{
        ...
        (Array.isArray(rows)?rows:[]).filter(row=>String(row.work_id||"").padStart(3,"0")==="007").forEach((row,index)=>{
          const page=Number(row.canvas_index||row.page||0);if(!page)return;
          if(!groups.has(page))groups.set(page,[]);
          const char=String(row.char||row.text||"").slice(0,1);
          groups.get(page).push({...row,work_id:"007",canvas_index:page,glyph_id:String(row.glyph_id||`007_${page}_${index+1}`),char,text:char,order_in_page:Number(row.order_in_page||index+1),bbox_x:Number(row.x??row.bbox_x??0),bbox_y:Number(row.y??row.bbox_y??0),bbox_w:Number(row.w??row.bbox_w??0),bbox_h:Number(row.h??row.bbox_h??0),bbox:[Number(row.x??row.bbox_x??0),Number(row.y??row.bbox_y??0),Number(row.w??row.bbox_w??0),Number(row.h??row.bbox_h??0)]});
        });
