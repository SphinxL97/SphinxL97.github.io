/* 作品004《麓山寺碑并阴》众智释读坐标适配。
 * 将已上传的逐页 IIIF 单字坐标转换为现有栏目四需要的分片数据格式。
 * 仅在 detail.html?id=004 时启用，不修改其他作品坐标。
 */
(function(){
  "use strict";
  ...
  if(parentId!=="004"||window.__WORK_004_COORDINATE_ADAPTER__) return;
  window.__WORK_004_COORDINATE_ADAPTER__=true;
  ...
  function normalizePage(boxes,pageNo){
    return (Array.isArray(boxes)?boxes:[]).map((box,index)=>{
      ...
      return {
        ...
        canvas_index:pageNo,
        page:pageNo,
        glyph_id:String(box.glyph_id||`004_${pageNo}_${index+1}`),
        ...
        canvas_width:number(box.canvas_width,1482),
        canvas_height:number(box.canvas_height,2212),
        x,y,w,h,
  ...
  async function buildRows(){
    const pageResponse=await nativeFetch("data/page_images_index.json?v=20260716_lushansi_coords_v1",{cache:"no-store"});
    if(!pageResponse.ok) throw new Error(`page index ${pageResponse.status}`);
    const pageData=await pageResponse.json();
    const pages=Array.isArray(pageData?.works?.["004"]?.pages)?pageData.works["004"].pages:[];
    const groups=await Promise.all(pages.map(async(page,index)=>{
      const pageNo=number(page.canvas_index||page.page,index+1);
      const path=`data/glyph_boxes/iiif/004/page_${String(pageNo).padStart(4,"0")}.json?v=20260716_lushansi_coords_v1`;
      try{
  ...
  window.fetch=async function(input,init){
    ...
    try{
      ...
      console.error("[work-004-coordinates] IIIF坐标转换失败",error);
