...
(function(){
  ...
  if(workId!=="036"||window.__WORK_036_COORDINATE_ADAPTER__)return;

  const CACHE_TAG="20260726_yiheming_036_v1";
  const ROOT="data/glyph_boxes/iiif/036";
  ...
  function normalizeRow(row,page,index){
    ...
    return {...row,work_id:"036",canvas_index:pageNo,glyph_id:String(row.glyph_id||`036_${pageNo}_${index+1}`),char:text,text,order_in_page:Number(row.order_in_page||row.annotation_index||index+1),bbox_x:box.x,bbox_y:box.y,bbox_w:box.w,bbox_h:box.h,bbox:[box.x,box.y,box.w,box.h]};
  }
  async function fetchRows(page){
    ...
    })().catch(error=>{pagePromises.delete(pageNo);console.warn("[work-036-coordinate-adapter]",pageNo,error);return[];});
  ...
  window.loadPageGlyphBoxes=async function(id,pageObj){
    ...
    if(normalized!=="036")return originalLoader?originalLoader(id,pageObj):[];
  ...
  window.WORK_036_COORDINATES={loadPageRows:fetchRows};
  window.__WORK_036_COORDINATE_ADAPTER__=true;
  window.dispatchEvent(new CustomEvent("work-036-coordinate-adapter-ready"));
})();
