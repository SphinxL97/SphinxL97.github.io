...
(function(){
  ...
  function markedHtml(value){
    ...
    while((match=pattern.exec(text))){html+=esc(text.slice(cursor,match.index));html+=`<span class="damage-added">〔${esc(match[1])}〕</span>`;cursor=match.index+match[0].length;}
    html+=esc(text.slice(cursor));return html;
  }
  ...
  function buildGapPatterns(original){
    ...
    for(let i=0;i<chars.length;){
      ...
      const candidates=[[5,5],[4,4],[3,3],[2,4],[4,2],[2,2]].map(([l,r])=>[...chars.slice(Math.max(0,i-l),i),...Array(end-i).fill("."),...chars.slice(end,Math.min(chars.length,end+r))].join("")).filter((value,index,array)=>value.replace(/\./g,"").length>=2&&array.indexOf(value)===index);
      patterns.push({patterns:candidates,gapLength:end-i});i=end;
    }
    return patterns;
  }
  ...
  function highlightAll(root,cases){
    ...
    root.querySelectorAll("p").forEach(paragraph=>{
      if(paragraph.dataset.work005AllHighlight==="true")return;
      ...
      if(!matches.length)return;matches.sort((a,b)=>a.index-b.index||b.end-a.end);
      const accepted=[];let cursor=-1;matches.forEach(match=>{if(match.index>=cursor){accepted.push(match);cursor=match.end;}});
      ...
      if(offset<text.length)fragment.appendChild(document.createTextNode(text.slice(offset)));paragraph.replaceChildren(fragment);paragraph.dataset.work005AllHighlight="true";
    });
  }
  async function renderTranscript(cases){
    ...
    const card=section.querySelector(".full-transcript-card");card.innerHTML=`<header class="full-transcript-header"><h3>${TITLE}</h3><span class="full-transcript-ornament" aria-hidden="true"></span></header><div class="full-transcript-body">${paragraphHtml(await response.text())}</div>`;
  ...
  async function loadPages(){
    ...
    await Promise.all(Array.from({length:8},async()=>{while(queue.length){const page=queue.shift(),pageNo=Number(page.page||page.canvas_index||0);if(!pageNo)continue;try{const response=await fetch(`data/glyph_boxes/iiif/005/page_${String(pageNo).padStart(4,"0")}.json?v=20260720_all_gaps_v1`,{cache:"force-cache"});if(!response.ok)continue;const boxes=(await response.json()).slice().sort((a,b)=>Number(a.order_in_page||0)-Number(b.order_in_page||0));if(!boxes.length)continue;const first=boxes[0];output.push({page:pageNo,image:page.image,boxes,chars:boxes.map(box=>normalizeChar(box.char||box.text||"")),canvas:{w:Number(first.canvas_width||1466),h:Number(first.canvas_height||2228)}});}catch(_){}}}));
    return output.sort((a,b)=>a.page-b.page);
  }
  function matchPattern(page,pattern){
    for(let start=0;start<=page.chars.length-pattern.length;start++){let ok=true;for(let i=0;i<pattern.length;i++){const p=pattern[i];if(p!=="."&&p!==page.chars[start+i]){ok=false;break;}}if(!ok)continue;const selected=[];for(let i=0;i<pattern.length;i++)if(pattern[i]===".")selected.push(page.boxes[start+i]);if(selected.length)return selected;}return null;
  }
  function geometry(page,selected){
    const rects=selected.map(rectOf).filter(r=>[r.x,r.y,r.w,r.h].every(Number.isFinite)&&r.w>0&&r.h>0);if(!rects.length)return null;
  ...
  async function resolveLocations(cases){
    try{const cached=JSON.parse(localStorage.getItem(CACHE_KEY)||"null");if(Array.isArray(cached)&&cached.length===cases.length){cases.forEach((item,index)=>item.locations=Array.isArray(cached[index])?cached[index]:[]);return;}}catch(_){}
    const pages=await loadPages();
    cases.forEach(item=>{const resolved=[];buildGapPatterns(item.original).forEach(group=>{let found=null;for(const pattern of group.patterns){for(const page of pages){const selected=matchPattern(page,pattern);if(!selected)continue;const geo=geometry(page,selected);if(geo){found={page:page.page,image:page.image,canvas:page.canvas,crop:geo.crop,target:geo.target};break;}}if(found)break;}if(found&&!resolved.some(existing=>locationKey(existing)===locationKey(found)))resolved.push(found);});item.locations=resolved;});
  ...
  function imageGallery(item){
    ...
    if(!locations.length)return `<div class="damage-location-missing"><p>现有逐字坐标中暂未自动匹配到该句的缺字区域，请在栏目一按原句继续核对。</p></div>`;
  ...
  function ensureStyle(){
    if(document.getElementById("work005-all-gap-style"))return;const style=document.createElement("style");style.id="work005-all-gap-style";
  ...
  async function init(){
    ensureStyle();const section=document.getElementById("people");if(section)section.innerHTML=`<h2 class="section-title">三、碑文残损与AI释读</h2><div class="damage-shell"><div class="full-transcript-loading">正在读取《${TITLE}》全部缺字案例并匹配现有字框……</div></div>`;
    try{const response=await fetch(CASE_URL,{cache:"no-store"});if(!response.ok)throw new Error(`${CASE_URL} ${response.status}`);cases=await response.json();await Promise.all([renderTranscript(cases),resolveLocations(cases)]);renderDamage();window.__WORK_005_CONTENT_READY__=true;window.dispatchEvent(new CustomEvent("work-005-content-ready"));}
    catch(error){console.error("[work-005-all]",error);if(section)section.innerHTML=`<h2 class="section-title">三、碑文残损与AI释读</h2><div class="damage-shell"><div class="full-transcript-error">《${TITLE}》全部缺字案例暂时无法读取，请刷新页面后重试。</div></div>`;}
  }
