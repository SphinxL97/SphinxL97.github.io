...
(function(){
  ...
  if(workId!=="032"||window.__WORK_032_COORDINATE_ADAPTER__)return;

  const CACHE_TAG="20260725_xuzhenren_032_v1";
  const ROOT="data/glyph_boxes/iiif/032";
  ...
  function normalizeRow(row,page,index){
    ...
    return {...row,work_id:"032",canvas_index:pageNo,glyph_id:String(row.glyph_id||`032_${pageNo}_${index+1}`),char:text,text,order_in_page:Number(row.order_in_page||row.annotation_index||index+1),bbox_x:box.x,bbox_y:box.y,bbox_w:box.w,bbox_h:box.h,bbox:[box.x,box.y,box.w,box.h]};
  }
  async function fetchRows(page){
    ...
    })().catch(error=>{pagePromises.delete(pageNo);console.warn("[work-032-coordinate-adapter]",pageNo,error);return[];});
  ...
  window.loadPageGlyphBoxes=async function(id,pageObj){
    ...
    if(normalized!=="032")return originalLoader?originalLoader(id,pageObj):[];
  ...
  window.WORK_032_COORDINATES={loadPageRows:fetchRows};
  window.__WORK_032_COORDINATE_ADAPTER__=true;
  window.dispatchEvent(new CustomEvent("work-032-coordinate-adapter-ready"));
})();
