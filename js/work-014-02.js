/* 014-02《颜真卿李玄靖碑册二》栏目二、三、四专属模块。 */
(function(){
  "use strict";

  const raw=String(new URLSearchParams(location.search).get("id")||"001");
  if(raw!=="014-02"||window.__WORK_014_02_LIHANJING__)return;
  window.__WORK_014_02_LIHANJING__=true;
  ...
  const PAGE_IMAGE_ROOT="assets/page_images/014_颜真卿李玄靖碑/images/02_顏真卿李玄靖碑册二";
  const NOTE="本节页面展示释文为由AI整理阅读版，段落划分和标点符号由AI辅助校对，仅供阅读参考。";
  const INTRO="本栏目以当前网页释文为底稿，只对原释文中明确标出的残损字提出校读意见。AI分析重点说明候选字与唐代道教语汇、颜真卿官历、人名、年号、铭文对偶及上下文的关系；没有对应方框的外部文字不写入恢复结果。";
  const COLOPHON="紹興丁巳五月十有四日，大風折顔碑，雲溪沈作舟扶起之。";
  ...
  function normalizeCase(row,index){
    ...
    return {...row,id,n:category,t:title,o:original,c:corrected,category,title,original,corrected,mode:String(row?.mode||"unresolved"),confidence:String(row?.confidence||"暂无法判断"),analysis:Array.isArray(row?.analysis)?row.analysis.map(String):[],locations,page:row?.page||locations[0]?.page||"—"};
  }

  function syncCases(items){
    window.DAMAGE_AI_CASES=items.map(item=>({...clone(item),id:item.id,n:item.category,t:item.title,o:item.original,c:item.corrected,category:item.category,title:item.title,original:item.original,corrected:item.corrected,analysis:[...(item.analysis||[])],locations:clone(item.locations||[]),page:item.page||"—"}));
    window.dispatchEvent(new CustomEvent("work-014-02-cases-ready",{detail:{count:items.length}}));
  }
  ...
  function boldProblemSentences(root,items){
    ...
    root.querySelectorAll("p").forEach(paragraph=>{
      ...
      accepted.forEach(item=>{
        ...
        const strong=document.createElement("strong");strong.className="transcript-problem-sentence";strong.textContent=value.slice(item.start,item.end);fragment.appendChild(strong);offset=item.end;
      });
  ...
  async function loadPageImages(){
    try{
      ...
      const list=Array.isArray(data?.works?.["014-02"]?.pages)?data.works["014-02"].pages:[];
      pageImages=new Map(list.map(page=>[Number(page.page),String(page.image||"")]));
    }catch(error){
      console.warn("[work-014-02] page index unavailable; deterministic paths remain active",error);
      pageImages=new Map();
    }
  }

  function chinesePageLabel(value){
    const page=Number(value||0),digits=["零","一","二","三","四","五","六","七","八","九"];
    if(page<=0)return "";
  ...
  function makeLocation(item){
    ...
    const bbox=source?.bbox,page=Number(source?.page||item.page||0);
  ...
  function markedHTML(value){
    const text=String(value||"");let html="",offset=0,match;const pattern=/〔([^〕]*)〕/g;
    while((match=pattern.exec(text))){html+=esc(text.slice(offset,match.index));html+=`<span class="damage-added">〔${esc(match[1])}〕</span>`;offset=match.index+match[0].length;}
  ...
  function renderDamage(){
    ...
    section.className="content-card damage-ai";section.dataset.work01402Dedicated="true";
  ...
  function ensureStyle(){
    if(document.getElementById("work014-02-lihanjing-style"))return;
    const style=document.createElement("style");style.id="work014-02-lihanjing-style";
    style.textContent=".damage-heading-confidence{font-size:.78em;color:#675b4e;white-space:nowrap}.damage-text.damage-new{color:#2e251e!important;font-weight:400!important}.damage-added{padding:0 .12em;border-bottom:2px solid #a53529;border-radius:4px;background:#f8e1cf;color:#9f3025!important;font-weight:900}.damage-location-missing{display:flex;align-items:center;justify-content:center;min-height:210px;padding:28px;border:1px dashed #d8c69f;border-radius:14px;background:#fffaf0;color:#7b6c5a}.volume-colophon-note{margin:28px 0 4px;padding:18px 22px;border:1px solid #decda8;border-radius:12px;background:#fffaf0}.volume-colophon-note h4{margin:0 0 8px;color:#963c30}.volume-colophon-note p{margin:0 0 8px;text-indent:0!important}.volume-colophon-note small{color:#756b60;line-height:1.7}";
  ...
  async function init(){
    ...
    try{
      const rows=await fetchJSON(CASE_URL);cases=(Array.isArray(rows)?rows:[]).map(normalizeCase);syncCases(cases);await renderTranscript(cases);renderDamage();loadPageImages().then(()=>renderDamage()).catch(()=>{});
      window.__WORK_014_02_CONTENT_READY__=true;window.__WORK_014_02_STABLE_READY__=true;
      window.dispatchEvent(new CustomEvent("work-014-02-content-ready",{detail:{count:cases.length}}));window.dispatchEvent(new CustomEvent("work-014-02-stable-ready",{detail:{cases:cases.length}}));
    }catch(error){
      console.error("[work-014-02]",error);window.__WORK_014_02_CONTENT_READY__=true;window.__WORK_014_02_STABLE_READY__=true;
