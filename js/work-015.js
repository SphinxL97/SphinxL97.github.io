/* 015《史晨前碑》栏目二、三、四专属模块。 */
(function(){
  "use strict";

  const raw=String(new URLSearchParams(location.search).get("id")||"001");
  const workId=(raw.includes("-")?raw.split("-")[0]:raw).padStart(3,"0");
  if(workId!=="015"||window.__WORK_015_SHICHENQIAN__)return;
  window.__WORK_015_SHICHENQIAN__=true;
  ...
  const TITLE="史晨前碑";
  const VERSION="20260723_shichenqian_v1";
  const TEXT_URL=`data/work015_full_text.txt?v=${VERSION}`;
  const CASE_URL=`data/work015_damage_cases.json?v=${VERSION}`;
  const NOTE="本节页面展示释文为由AI整理阅读版，段落划分和标点符号由AI辅助校对，仅供阅读参考。";
  const INTRO="本栏目以当前网页释文为底稿，只对原释文中明确标出的残损字提出校读意见。AI分析重点说明候选字与孔子周游、谶纬典故、句法和铭辞对偶的关系；没有对应方框的外部文字不写入恢复结果。";
  const IMAGE_ROOT="assets/page_images/015_史晨前碑/images";
  ...
  function syncCases(items){
    ...
    window.dispatchEvent(new CustomEvent("work-015-cases-ready",{detail:{count:items.length}}));
  }
  ...
  async function renderTranscript(items){
    ...
    try{
      ...
      console.error("[work-015] transcript",error);
  ...
  function renderDamage(){
    ...
    section.dataset.work015Dedicated="true";
  ...
  function ensureStyle(){
    if(document.getElementById("work015-shichenqian-style"))return;
    const style=document.createElement("style");
    style.id="work015-shichenqian-style";
  ...
  async function init(){
    ...
    try{
      ...
      renderDamage();
      window.__WORK_015_CONTENT_READY__=true;
      window.__WORK_015_STABLE_READY__=true;
      window.dispatchEvent(new CustomEvent("work-015-content-ready",{detail:{count:cases.length}}));
      window.dispatchEvent(new CustomEvent("work-015-stable-ready",{detail:{cases:cases.length}}));
    }catch(error){
      console.error("[work-015]",error);
      window.__WORK_015_CONTENT_READY__=true;
      window.__WORK_015_STABLE_READY__=true;
